...
class Boid:
    ...
    def update(self, boids: Set["Boid"], flockmates: Set["Boid"], dt: float) -> None:
        ...
        local_flockmates: Set["Boid"]
        self.local_boids, local_flockmates = self._get_local_boids_and_flockmates(boids, flockmates)
        
        self._avoid_local_boids(self.local_boids, dt)
        self._align_with_local_flockmates(local_flockmates, dt)
        self._steer_towards_local_flockmates(local_flockmates, dt)
    ...
    def draw(self, screen: pg.Surface, colour: pg.Color = pg.Color("White")) -> None:
        ...
        if self.selected:
            for boid in self.local_boids:
                pg.draw.line(screen, pg.Color("Yellow"), self.position, boid.position)
    ...
    def _get_local_boids_and_flockmates(self, boids: Set["Boid"], flockmates: Set["Boid"]) -> Set["Boid"]:
        local_boids: Set["Boid"] = set()
        local_flockmates: Set["Boid"] = set()
        ...
        for boid in boids:
            ...
            to_boid: pg.Vector2 = (boid.position - self.position).normalize()
            forward: pg.Vector2 = self.velocity.normalize()
            dot_product: float = forward.dot(to_boid) # dot(A,B) = ∥A∥ ∥B∥ cos(θ)
            cos_half_vision: float = cos(radians(self.half_vision_angle))

            if dot_product >= cos_half_vision:
                local_boids.add(boid)
                
                if boid in flockmates:
                    local_flockmates.add(boid)
                
        return local_boids, local_flockmates
    ...
    # Separation
    def _avoid_local_boids(self, local_boids: Set["Boid"], dt: float) -> None:
    ...
    # Alignment
    def _align_with_local_flockmates(self, local_flockmates: Set["Boid"], dt: float) -> None:
        velocity_acc: pg.Vector2 = pg.Vector2(0, 0)
        neighbouring_boids: int = 0
        
        for mate in local_flockmates:
        ...
        if neighbouring_boids > 0:
            ...
            alignment_force: pg.Vector2 = (velocity_avg - self.velocity).normalize() * self._config.align_speed
            self.apply_force(alignment_force, dt)
            
    # Cohesion
    def _steer_towards_local_flockmates(self, local_flockmates: Set["Boid"], dt: float) -> None:
